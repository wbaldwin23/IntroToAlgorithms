...
public class Autocomplete {
   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
         ...
         if ( word == null) {
            throw new NullPointerException();
                                              ...
                                              //Checking if word and weight are valid for method.
         if (weight < 0) {
         ...
         for ( char a : word.toCharArray())  {      //Checking for corresponding node
            Node child = node.getChild(a);
            if (node.mySubtreeMaxWeight < weight) {    //Updating weight if needed
               node.mySubtreeMaxWeight = weight;
            }
           
            if (child != null) {            //Going down tree to find opening
               node = node.getChild(a);
            } 
            
            else {
               child = new Node(a, node, weight); //Adding the new node to the tree
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         if (prefix == null) {                 // Checking if prefix put in is sufficient for the method.
            throw new NullPointerException();
         }
         Node node = myRoot;
         PriorityQueue<Node> nodeQueue = new PriorityQueue<>(new Node.ReverseSubtreeMaxWeightComparator());
         ...
         for (int i = 0; i < prefix.length(); i++) {
            a = prefix.charAt(i);               //Going through the characters of prefix
            node = node.getChild(a);            //to get matching node and adding it.
            if (node == null) {
               return stringList;               //If the node is null then that means there
            }                                   // is no more words in subtree for use.
         }nodeQueue.add(node);
          
         do {
            node = nodeQueue.poll();         
            if (node.isWord) {            //Checking if valid word to add and if is then added
               stringList.add(node.myWord);
            }
            if (stringList.size() == k) {    //If we reach the k value of matches, then we stop
               break;
            }
            for(Node child : node.children.values()) {  //Adding more values to be checked for topmatches
               nodeQueue.add(child);
            }  
      ...
      public String topMatch(String prefix) {
         ...
         Node node = myRoot;
         char a;
         for (int i = 0; i < prefix.length(); i++) {    //Going through each character and finding
            a = prefix.charAt(i);                       // the node that corresponds to it.
            node = node.getChild(a);
            if (node == null) {
               return "";
            }
         }
         double maxWeight = node.mySubtreeMaxWeight;    //Using mysubtreemaxweight to compare and get the top match.
         
         if (node.children.size() == 0) {      // If node has no children, then it is the word. 
            return node.myWord;
         }
         ...
         do {
            ...
            for (Node child: node.children.values()) {   
               ...
               if (child.mySubtreeMaxWeight == maxWeight) {   //Going through each value of the children
                  node = child;                               //and if they are the max weight of the tree, 
                  break;                                      //then that is the the top match.
               }
            
            }
         }while (node.myWeight != maxWeight); 
      ...
      public double weightOf(String term) {
         ...
         if (term == null) {       
            return 0.0; 
         }                             // Checking if term put in is sufficient for the method.
         if (term.length() == 0) {
            return 0.0;
         }
         ...
         for ( char a : term.toCharArray())  {
            Node child = node.getChild(a);
            if (child != null) {                  // Going through the term by each character                              
               node = child;                      // to find the node with the weight.
            }                                     // If the node is null returns zero.
            else {
               return 0.0;
            }
